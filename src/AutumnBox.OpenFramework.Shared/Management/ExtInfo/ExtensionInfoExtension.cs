...
namespace AutumnBox.OpenFramework.Management.ExtInfo
{
    ...
    public static class ExtensionInfoExtension
    {
        ...
        public static byte[] Icon(this IExtensionInfo extensionInfo)
        {
            try
            {
                string iconSource = extensionInfo.Metadata[ExtensionMetadataKeys.ICON]?.Invoke() as string ?? DEFAULT_ICON;
            ...
            {
                return Convert.FromBase64String(DEFAULT_ICON);
            }
            ...
            static byte[]? ReadAsResource(ClassExtensionInfo? ceinf, string source)
            {
                try
                {
                    if (ceinf != null)
                    {
                        ...
                        Stream stream = ceinf?.ClassExtensionType?.Assembly?.GetManifestResourceStream(path) ?? throw new FileNotFoundException();
